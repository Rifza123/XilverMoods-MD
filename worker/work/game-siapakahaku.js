...
let siapakahaku = JSON.parse(fs.readFileSync('./lib/container/siapakahaku.json'))
	 ...
	 if(!global.use[msg.sender]?.game?.time){
      ...
      return
         ...
         await addGame(from)
            ...
            let data = siapakahaku[Math.floor(Math.random() * siapakahaku.length)];
            let jawaban = data.jawaban
             console.log(data)
            global._siapakahaku[from.split('@')[0]] = { date: date, user: from, jawaban: jawaban.toLowerCase(), time: waktu, coin: upah }  
             fs.writeFileSync("./lib/container/database/siapakahaku.json", JSON.stringify(global._siapakahaku, null, 2))
            ...
            await sleep(global._siapakahaku[from.split('@')[0]].time)      
            let isGame = global._siapakahaku.hasOwnProperty(from.split('@')[0])   
            
            if (isGame && _siapakahaku[from.split("@")[0]].date == date) {
